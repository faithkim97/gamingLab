...
public interface GameRepository extends CrudRepository<Game, Integer> {
    ...
    @Query(value = full_table + "where game.title like %?1% or genre.genre like %?1% or game.description like %?1% and game.is_checked_out = ?2 "
            +"and game.is_digital = ?3 and console.console = ?4 and game.rating = ?5 and playable_mode.mode = ?6", nativeQuery = true)
